...
wordpress-theme-starter.zip: clean-release build-assets
	zip -r wordpress-theme-starter.zip ./theme
...
clean-release:
	...
	rm -f wordpress-theme-starter.zip
...
ifdef HAS_LANDO
	...
	if [ ! "$$(docker ps | grep wordpressthemestarter_appserver)" ]; then \
		echo "Starting Lando"; \
		lando start; \
	fi
...
.PHONY: relase
release: wordpress-theme-starter.zip
...
setup-wordpress:
	...
	lando wp core install --path=./wordpress --url=https://wordpress-theme-starter.lndo.site --title="WordPress Theme Development" --admin_user=admin --admin_password=password --admin_email=no@noway.com
